...
{
	const manifest = JSON.parse(readFileSync(join(SPC_ASSETS, "driver", "manifest.json"), "utf8")) as {
	...
	for (const name of names) {
		const raw = new Uint8Array(readFileSync(join(SPC_ASSETS, "driver", "samples", name)));
