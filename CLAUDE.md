...
separation and contrast against `--color-surface` are the properties being preserved. The global
`:focus-visible` outline must not be removed by components — it is what makes the editor navigable
from a keyboard at all, which is not the same concern as the one below.
...
`web/.claude/CLAUDE.md` does not apply here. That file is generated by the Angular CLI and is
deliberately left byte-identical to what the CLI emits, so the override is stated here instead.
