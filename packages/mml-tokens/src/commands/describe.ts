...
import { type Control, type ParamChoice, type ParamContext, type ParamDescriptor, raw, toSigned } from "./param";
...
export function resolveCommand(command: Command, context: ParamContext): ResolvedCommand {
	const resolver = command.vcmd !== undefined ? HEX_PARAMS[command.vcmd] : LETTER_PARAMS[command.kind.toLowerCase()];

	const shape = resolver ? resolver(command, context) : { params: [], note: undefined };

	const expected = command.vcmd !== undefined ? expectedArgs(command.vcmd, command.args, command.target) : null;
	...
	for (let index = 0; index < shown; index++) {
		...
		rows.push({
			...
			lockedBecause: missing ? "not written yet" : macro !== undefined ? `comes from the "${macro}" replacement` : null,
		});
	}
