...
console.log('\nevery region that exists stays visible');
{
	...
	check('no segment is ever wider than the bar', place(REALISTIC).every((p) => p.width <= OPTS.width));
}
...
console.log('\ndegenerate input produces no geometry rather than bad geometry');
{
	...
	check('a cramped bar has no negative widths', cramped.every((p) => p.width >= 0));
