...
@Component({
  ...
  template: `
    <button
      type="button"
      class="border-edge bg-inset text-ink-muted hover:text-ink hover:border-control shrink-0 cursor-pointer rounded-md border px-2 py-1 font-mono text-xs disabled:cursor-not-allowed disabled:opacity-40"
      [disabled]="disabled()"
      (click)="toggle()"
    >
      {{ sounding() ? '■' : '▶' }}
